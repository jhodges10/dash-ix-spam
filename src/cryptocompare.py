...
class CryptoCompare:
    ...
    def fetch_price_history_dict(use_cache=True, days_to_fetch=1200):
        ...
        if use_cache and os.path.isfile(storage_location):
            ...
            CryptoCompare.c_write_json(day_and_price_dict, "price_history")
    ...
    @staticmethod
    # @lru_cache() 
    # If caching this function and an exception is thrown where we're missing a date, we keep pulling the same data
    def c_read_json(filename):
        cache_dir = '_cache/'
        dirname = os.path.dirname(os.path.abspath(__file__))
        absolute_cache_dir = os.path.abspath(os.path.join(dirname, cache_dir))
        filename = filename + '.json'
