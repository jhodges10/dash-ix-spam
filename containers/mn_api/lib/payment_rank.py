from lib.rpc_methods import get_prgtx_info, get_best_block
...
def get_payment_ranks():
    ...
    for node in proregtx_info_list:
        ...
        rank = round((abs((payment_distance / node_count)) * 100), 2)


        node_dict[vin] = rank

    sorted_set = OrderedDict(sorted(node_dict.items(), key=lambda x: x[1], reverse=False))
